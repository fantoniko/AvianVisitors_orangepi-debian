(function () {
  ...
  var SKETCH_VERSION = 'r11'; // external full pose mask manifests
  ...
  function loadMaskData() {
    var version = encodeURIComponent(SKETCH_VERSION);
    return Promise.all([
      fetch('./dims.json?v=' + version).then(function (r) {
        return r.ok ? r.json() : Promise.reject(r.status);
      }),
      fetch('./masks.json?v=' + version).then(function (r) {
        return r.ok ? r.json() : Promise.reject(r.status);
      }),
    ]).then(function (parts) {
      if (!parts[0] || !parts[1]) throw new Error('empty mask manifest');
      DIMS = parts[0];
      MASKS = parts[1];
      return true;
    }).catch(function (error) {
      console.warn('mask manifests failed to load', error);
      return false;
    });
  }
  ...
  function loadMask(slug) {
    ...
    var rec = MASKS[slug];
    if (!rec) return null;
    var encoded = atob(rec.bits);
    var bits = new Uint8Array(encoded.length);
    for (var bi = 0; bi < encoded.length; bi++) bits[bi] = encoded.charCodeAt(bi);
    var w = rec.w, h = rec.h;
    var opaque = 0;
    for (var pi = 0; pi < w * h; pi++) {
      if ((bits[pi >> 3] >> (7 - (pi & 7))) & 1) opaque++;
    }
    // Flat x,y pairs avoid one heap-allocated JavaScript array per opaque
    // cell. Uint16 is ample for the <=93px mask dimensions.
    var cells = new Uint16Array(opaque * 2);
    var ci = 0;
    for (var y = 0; y < h; y++) {
      for (var x = 0; x < w; x++) {
    ...
    // Release this mask's base64 payload after decoding. The packed bytes are
    // retained for constant-time pointer hit-testing.
    delete MASKS[slug];
    return (maskCache[slug] = { w: w, h: h, cells: cells, bits: bits });
  }
  ...
  function maskPack(tiles, W, H, xBias, yBias, pad) {
    ...
    function cellRange(tile, tx, ty, cellX, cellY) {
      // For a mask cell, return [gx0, gy0, gx1, gy1] (inclusive)
      // in grid coords, clamped to the grid.
      var sx = tile.fullW / tile.mask.w;
      var sy = tile.fullH / tile.mask.h;
      var x0 = (tx + cellX * sx) / GRID_STRIDE | 0;
      var y0 = (ty + cellY * sy) / GRID_STRIDE | 0;
      var x1 = (tx + (cellX + 1) * sx) / GRID_STRIDE | 0;
      var y1 = (ty + (cellY + 1) * sy) / GRID_STRIDE | 0;
    ...
    function collides(tile, tx, ty) {
      ...
      for (var i = 0; i < cells.length; i += 2) {
        var r = cellRange(tile, tx, ty, cells[i], cells[i + 1]);
  ...
  function maskHitTest(clientX, clientY) {
    ...
    for (var i = collagePlaced.length - 1; i >= 0; i--) {
      ...
      var mi = my * t.mask.w + mx;
      if ((t.mask.bits[mi >> 3] >> (7 - (mi & 7))) & 1) return t;
    }
    return null;
  }
  ...
  function allTimeRecent() {
    var lifelist = DATA.lifelist || { species: [] };
    return {
      hours: ALL_HOURS,
      species: (lifelist.species || []).slice().sort(function (a, b) {
        return String(b.last_seen || '').localeCompare(String(a.last_seen || ''));
      }),
      as_of: lifelist.as_of || null,
    };
  }

  function firstSeenFromLifelist(limit) {
    var lifelist = DATA.lifelist || { species: [] };
    return {
      species: (lifelist.species || []).slice().sort(function (a, b) {
  ...
  // API responses include an `as_of` timestamp that changes on every poll.
  // Build render keys from visible data only, so an otherwise identical
  // response does not trigger the expensive collage packing and full DOM
  // rebuild every 30 seconds.
  function statsRenderKey(stats) {
    if (!stats) return '';
  ...
  function recentRenderKey(recent) {
    if (!recent) return '';
    return JSON.stringify([recent.hours || null, recent.species || []]);
  }
  function fullRenderKey(data) {
    return JSON.stringify([
      statsRenderKey(data.stats),
      (data.lifelist && data.lifelist.species) || [],
      data.timeseries ? [data.timeseries.days, data.timeseries.daily || [], data.timeseries.by_hour || []] : null,
      (data.firstseen && data.firstseen.species) || [],
      recentRenderKey(data.recent),
    ]);
  }
  ...
  function fetchLiveData(hours) {
    if (hours >= ALL_HOURS) {
      return fetchJson('./avian/api/birdnet-api.php?action=stats').then(function (stats) {
  ...
  function refreshRecent(animate) {
    ...
    if (forHours >= ALL_HOURS) {
      DATA.recent = allTimeRecent();
  ...
  function refreshAll(animate, renderGate) {
    var forHours = currentHours;
    var previousRenderKey = fullRenderKey(DATA);
    return Promise.all([
      fetchLiveData(forHours).catch(function () { return null; }),
      ...
      var live = parts[0] || { stats: null, recent: null };
      DATA.stats = live.stats;
      DATA.lifelist = parts[1];
      DATA.timeseries = parts[2];
      DATA.firstseen = firstSeenFromLifelist(10);
      ...
      if (forHours === currentHours) {
        if (forHours >= ALL_HOURS) DATA.recent = allTimeRecent();
        else if (live.recent) DATA.recent = live.recent;
      }
      return Promise.resolve(renderGate).then(function () {
        if (previousRenderKey === fullRenderKey(DATA)) return false;
        recomputeDerived();
  ...
  function refreshLive(animate) {
    var forHours = currentHours;
    var previousStatsKey = statsRenderKey(DATA.stats);
    var previousRecentKey = recentRenderKey(DATA.recent);
    return fetchLiveData(forHours).then(function (parts) {
      if (parts.stats) DATA.stats = parts.stats;
      if (forHours === currentHours && parts.recent) DATA.recent = parts.recent;
      var statsChanged = previousStatsKey !== statsRenderKey(DATA.stats);
      var recentChanged = previousRecentKey !== recentRenderKey(DATA.recent);
      if (!statsChanged && !recentChanged) return false;
      renderWindowDependent(animate);
      return true;
  ...
  // Start local manifests and remote API requests together, but gate the
  // first render on the manifests so the collage never flashes empty.
  refreshAll(true, loadMaskData());
